...
const SEED = {
  weeks: [
    ...
    },
    {
      ...
      chapters: "Caps. 14 y 15",
      pc: "",
      blurb: "Cómo la deuda redistribuye riesgo y retorno, cuándo crea valor mediante impuestos y por qué el apalancamiento tiene límites.",
      sections: [
...
El objetivo no es memorizar fórmulas aisladas, sino mantener una identidad económica coherente: los activos determinan el riesgo del negocio; la deuda redistribuye ese riesgo; los impuestos pueden añadir valor; y el exceso de deuda reduce flexibilidad financiera.`, widget: "capital-case" },
      ...
      quiz: [
        ...
        { id:"w4q2", q:"¿Qué permite al inversionista replicar una estructura de capital distinta a la elegida por la empresa?", o:["El escudo fiscal","El apalancamiento interno","La depreciación","La recompra sin deuda"], a:1, e:"El inversionista puede prestar o pedir prestado por su cuenta para replicar los flujos deseados." },
        { id:"w4q3", q:"Según MM II sin impuestos, al aumentar D/E:", o:["rE disminuye","rE aumenta","rU aumenta","El WACC cae siempre"], a:1, e:"El equity absorbe más riesgo financiero y exige una prima mayor." },
        { id:"w4q4", q:"¿Por qué el WACC permanece constante en mercados perfectos?", o:["Porque rD es siempre cero","Porque el mayor peso de deuda es compensado por un mayor costo del equity","Porque la empresa deja de tener riesgo","Porque los impuestos son negativos"], a:1, e:"La deuda barata no es una ganancia gratuita: el apalancamiento eleva rE." },
        { id:"w4q5", q:"Si βD = 0 y una empresa aumenta D/E manteniendo el mismo negocio, ¿qué ocurre normalmente con βE?", o:["Disminuye","Aumenta","Permanece igual a βU","Se vuelve cero"], a:1, e:"La beta de los activos permanece; la beta del equity sube para reflejar el riesgo financiero." },
        { id:"w4q6", q:"El escudo fiscal de intereses de un periodo es:", o:["EBIT × deuda","Tasa impositiva × intereses","WACC × deuda","Intereses ÷ tasa impositiva"], a:1, e:"La deducción de intereses evita pagar T por cada unidad de interés deducible." },
        { id:"w4q7", q:"Con deuda permanente y constante, el valor del escudo fiscal es:", o:["T × D","rD × D","T × EBIT","D/E"], a:0, e:"Bajo esos supuestos, la perpetuidad de escudos fiscales vale T × D." },
        { id:"w4q8", q:"¿Por qué el beneficio fiscal de la deuda tiene un límite?", o:["Porque la deuda nunca paga intereses","Porque no puede deducirse más interés que la renta gravable disponible","Porque el equity deja de existir","Porque rU siempre cae"], a:1, e:"Cuando los intereses superan el EBIT, parte del escudo no puede utilizarse en ese periodo." },
        { id:"w4q9", q:"Una recapitalización apalancada usa deuda para recomprar acciones. En MM I, el precio por acción inmediatamente después de la recompra:", o:["Debe duplicarse","Debe caer a cero","Permanece igual si no cambian los activos","Es igual al valor contable"], a:2, e:"Cae el equity total y también el número de acciones; el precio por acción no cambia por la transacción financiera." },
        { id:"w4q10", q:"La diferencia conceptual correcta es:", o:["βU incluye riesgo financiero y βE no","βU mide el riesgo del negocio; βE añade el efecto del apalancamiento","Ambas betas son siempre iguales","βE mide únicamente riesgo idiosincrático"], a:1, e:"La beta desapalancada pertenece a los activos; la apalancada corresponde al equity." },
      ],
    },
...
function WCapitalSplit(){
  const [debt,setDebt]=useState(500), asset=1000, rd=.05, strong=1400, weak=900;
  const repay=debt*(1+rd), equity=asset-debt;
  return <Widget title="Cómo se reparten los flujos" hint="La deuda cambia el reparto, no el flujo generado por los activos.">
    <Slider label="Deuda emitida hoy" value={debt} min={0} max={800} step={25} onChange={setDebt} suffix="$"/>
    <div className="capital-balance"><div style={{width:`${debt/asset*100}%`}}><span>Deuda</span><b>${fmt(debt,0)}</b></div><div style={{width:`${equity/asset*100}%`}}><span>Equity</span><b>${fmt(equity,0)}</b></div></div>
    <div className="scenario-grid"><div><span>Economía fuerte · activos ${fmt(strong,0)}</span><b>Deuda ${fmt(Math.min(repay,strong),0)}</b><small>Equity ${fmt(Math.max(strong-repay,0),0)}</small></div><div><span>Economía débil · activos ${fmt(weak,0)}</span><b>Deuda ${fmt(Math.min(repay,weak),0)}</b><small>Equity ${fmt(Math.max(weak-repay,0),0)}</small></div></div>
    <div className="metric-row"><div><span>Valor de activos</span><b>$1,000</b></div><div><span>D + E</span><b>${fmt(debt+equity,0)}</b></div><div className="accent"><span>Identidad</span><b>V = D + E</b></div></div>
  </Widget>;
}
function WMM1Recap(){
  const [debt,setDebt]=useState(80), shares0=50, price=4, value=shares0*price, repurchased=debt/price, shares=shares0-repurchased, equity=value-debt;
  return <Widget title="Recapitalización apalancada" hint="Harrison Industries: deuda para recomprar acciones en un mercado perfecto.">
    <Slider label="Nueva deuda" value={debt} min={0} max={160} step={5} onChange={setDebt} suffix=" MM"/>
    <div className="recap-flow"><div><span>Antes</span><b>{shares0} MM acciones</b><small>${price} por acción · Equity ${value} MM</small></div><i>→</i><div><span>Recompra</span><b>{fmt(repurchased,1)} MM acciones</b><small>financiadas con ${debt} MM de deuda</small></div><i>→</i><div className="accent"><span>Después</span><b>{fmt(shares,1)} MM acciones</b><small>Equity ${fmt(equity,0)} MM · ${fmt(equity/shares,2)} por acción</small></div></div>
    <div className="formula-callout">El valor de los activos sigue siendo <b>${value} MM</b>. La operación reduce E exactamente en el monto de D.</div>
  </Widget>;
}
function WMM2Wacc(){
  const [de,setDe]=useState(50),[ru,setRu]=useState(15),[rd,setRd]=useState(5); const re=ru+(de/100)*(ru-rd), wd=de/(100+de), we=1-wd, wacc=we*re+wd*rd;
  const pts=[0,25,50,75,100,150,200].map(x=>({x,re:ru+x/100*(ru-rd)}));
  return <Widget title="MM II y WACC constante" hint="Aumenta D/E: la deuda gana peso, pero el equity exige más retorno.">
    <div className="grid2"><Slider label="D/E" value={de} min={0} max={200} step={5} onChange={setDe} suffix="%"/><Slider label="rU" value={ru} min={6} max={25} step={.5} onChange={setRu} suffix="%" d={1}/><Slider label="rD" value={rd} min={0} max={15} step={.5} onChange={setRd} suffix="%" d={1}/></div>
    <div className="metric-row four"><div><span>rE</span><b>{fmt(re,2)}%</b></div><div><span>D/V</span><b>{fmt(wd*100,1)}%</b></div><div><span>E/V</span><b>{fmt(we*100,1)}%</b></div><div className="accent"><span>WACC</span><b>{fmt(wacc,2)}%</b></div></div>
    <svg className="mini-chart" viewBox="0 0 620 190"><line x1="45" y1="155" x2="590" y2="155"/><line x1="45" y1="20" x2="45" y2="155"/><line className="wacc-flat" x1="45" y1={155-ru*5} x2="590" y2={155-ru*5}/><polyline className="equity-rise" points={pts.map(p=>`${45+p.x*2.7},${155-p.re*5}`).join(' ')}/><text x="500" y={148-ru*5}>WACC = rU</text><text x="500" y={145-(ru+2*(ru-rd))*5}>rE</text></svg>
  </Widget>;
}
function WCapitalBeta(){
  const [betaE,setBetaE]=useState(.8),[de0,setDe0]=useState(.1),[de1,setDe1]=useState(.5); const betaU=betaE/(1+de0), betaNew=betaU*(1+de1);
  return <Widget title="Desapalancar y reapalancar beta" hint="Supuesto didáctico: beta de la deuda igual a cero.">
    <div className="grid2"><Slider label="βE observada" value={betaE} min={.2} max={2} step={.05} onChange={setBetaE} d={2}/><Slider label="D/E actual" value={de0} min={0} max={1.5} step={.05} onChange={setDe0} d={2}/><Slider label="D/E objetivo" value={de1} min={0} max={2} step={.05} onChange={setDe1} d={2}/></div>
    <div className="beta-flow"><div><span>Equity actual</span><b>βE {fmt(betaE,2)}</b></div><span>desapalancar</span><div className="accent"><span>Activos</span><b>βU {fmt(betaU,2)}</b></div><span>reapalancar</span><div><span>Equity objetivo</span><b>βE {fmt(betaNew,2)}</b></div></div>
  </Widget>;
}
function WTaxShield(){
  const [ebit,setEbit]=useState(2800),[interest,setInterest]=useState(400),[tax,setTax]=useState(35); const shield=Math.min(ebit,interest)*tax/100, taxesNo=ebit*tax/100, taxesWith=Math.max(ebit-interest,0)*tax/100;
  return <Widget title="Ahorro fiscal generado por la deuda" hint="El interés reduce la renta gravable hasta el límite del EBIT disponible.">
    <div className="grid2"><Slider label="EBIT" value={ebit} min={200} max={5000} step={100} onChange={setEbit} suffix=" MM"/><Slider label="Intereses" value={interest} min={0} max={3500} step={50} onChange={setInterest} suffix=" MM"/><Slider label="Impuesto" value={tax} min={0} max={45} step={1} onChange={setTax} suffix="%"/></div>
    <div className="tax-compare"><div><span>Sin deuda</span><b>Impuestos ${fmt(taxesNo,0)}</b><small>Base gravable ${fmt(ebit,0)}</small></div><div><span>Con deuda</span><b>Impuestos ${fmt(taxesWith,0)}</b><small>Base gravable ${fmt(Math.max(ebit-interest,0),0)}</small></div><div className="accent"><span>Escudo utilizado</span><b>${fmt(shield,0)}</b><small>{interest>ebit?`Intereses exceden EBIT en ${fmt(interest-ebit,0)}`:"Escudo completamente utilizable"}</small></div></div>
  </Widget>;
}
function WCapitalOptimum(){
  const [ebit,setEbit]=useState(1000),[rate,setRate]=useState(8),[tax,setTax]=useState(30),[debt,setDebt]=useState(5000); const interest=debt*rate/100, shield=Math.min(interest,ebit)*tax/100, use=Math.min(interest/ebit,1)*100;
  return <Widget title="Hasta dónde llega el beneficio fiscal" hint="El modelo muestra el límite tributario; los costos de quiebra se desarrollan posteriormente.">
    <div className="grid2"><Slider label="Deuda" value={debt} min={0} max={18000} step={250} onChange={setDebt} suffix=""/><Slider label="Tasa de interés" value={rate} min={1} max={18} step={.5} onChange={setRate} suffix="%" d={1}/><Slider label="EBIT esperado" value={ebit} min={200} max={2500} step={50} onChange={setEbit}/><Slider label="Impuesto" value={tax} min={0} max={45} step={1} onChange={setTax} suffix="%"/></div>
    <div className="limit-bar"><span style={{width:`${Math.min(use,100)}%`}}></span><i style={{left:`${Math.min(use,100)}%`}}></i></div><div className="limit-labels"><span>Intereses 0</span><b>{fmt(use,0)}% del EBIT</b><span>Intereses = EBIT</span></div>
    <div className="metric-row"><div><span>Intereses</span><b>{fmt(interest,0)}</b></div><div><span>Escudo fiscal</span><b>{fmt(shield,0)}</b></div><div className={interest>ebit?"warnbox":"accent"}><span>Lectura</span><b>{interest>ebit?"Escudo limitado":"Margen tributario"}</b></div></div>
  </Widget>;
}
function WCapitalCase(){
  const [vu,setVu]=useState(1000),[debt,setDebt]=useState(300),[ru,setRu]=useState(12),[rd,setRd]=useState(6),[tax,setTax]=useState(30); const pvts=tax/100*debt, vl=vu+pvts, e=vl-debt, de=debt/e, re=ru+de*(ru-rd)*(1-tax/100), wd=debt/vl, we=e/vl, wacc=we*re+wd*rd*(1-tax/100);
  return <Widget title="Caso integrador" hint="Deuda permanente y constante; MM II ajustada por impuestos.">
    <div className="grid2"><Slider label="VU" value={vu} min={400} max={2500} step={50} onChange={setVu}/><Slider label="Deuda" value={debt} min={0} max={1200} step={25} onChange={setDebt}/><Slider label="rU" value={ru} min={5} max={22} step={.5} onChange={setRu} suffix="%" d={1}/><Slider label="rD" value={rd} min={1} max={15} step={.5} onChange={setRd} suffix="%" d={1}/><Slider label="Impuesto" value={tax} min={0} max={45} step={1} onChange={setTax} suffix="%"/></div>
    <div className="capital-dashboard"><div><span>VP escudo</span><b>{fmt(pvts,0)}</b><small>T × D</small></div><div><span>VL</span><b>{fmt(vl,0)}</b><small>VU + VP escudo</small></div><div><span>Equity</span><b>{fmt(e,0)}</b><small>VL − D</small></div><div><span>rE</span><b>{fmt(re,2)}%</b><small>riesgo apalancado</small></div><div className="accent"><span>WACC</span><b>{fmt(wacc,2)}%</b><small>después de impuestos</small></div></div>
  </Widget>;
}
...
const WIDGETS = {
  ...
  "capital-split": WCapitalSplit,
  "mm1-recap": WMM1Recap,
  "mm2-wacc": WMM2Wacc,
  "capital-beta": WCapitalBeta,
  "tax-shield": WTaxShield,
  "capital-optimum": WCapitalOptimum,
  "capital-case": WCapitalCase,
};
const WIDGET_LIST = [
  ["", "— sin interactivo —"],
...
export default function App() {
  ...
  return (
    <div className="root">
      ...
      <div className="persistent-notice">Sujeto a errores y omisiones. Para reportar cualquier error u omisión, escribir a <a href="mailto:ge.montesa@up.edu.pe">ge.montesa@up.edu.pe</a></div>
        </main>
      )}
...
/* Semana 4 */
.persistent-notice{position:sticky;top:64px;z-index:25;background:#fff8df;border-bottom:1px solid #ead58a;color:#55440b;text-align:center;padding:8px 18px;font-size:11px;font-weight:650;letter-spacing:.01em}.persistent-notice a{color:inherit;text-decoration:underline;text-underline-offset:2px}
.capital-balance{display:flex;height:76px;border-radius:12px;overflow:hidden;margin:16px 0;background:var(--wash)}.capital-balance>div{display:flex;min-width:55px;flex-direction:column;align-items:center;justify-content:center;color:#fff;background:var(--amber);transition:width .25s}.capital-balance>div:last-child{background:var(--blue)}.capital-balance span{font-size:10px}.capital-balance b{font-family:var(--disp);font-size:20px}.recap-flow{display:flex;align-items:stretch;gap:10px;margin-top:15px}.recap-flow>div{flex:1;border:1px solid var(--line);border-radius:11px;padding:14px;background:#fff}.recap-flow>i{display:flex;align-items:center;color:var(--mute);font-style:normal}.recap-flow span,.recap-flow small{display:block;color:var(--mute);font-size:10.5px}.recap-flow b{display:block;font-family:var(--disp);font-size:20px;margin:4px 0}.formula-callout{margin-top:14px;padding:13px 15px;border-radius:10px;background:var(--wash);font-size:12px}.mini-chart{width:100%;height:190px;margin-top:14px;overflow:visible}.mini-chart line{stroke:var(--line);stroke-width:1}.mini-chart .wacc-flat{stroke:var(--go);stroke-width:2.4;stroke-dasharray:6 4}.mini-chart .equity-rise{fill:none;stroke:var(--blue);stroke-width:2.6}.mini-chart text{font-size:10px;fill:var(--mute)}.tax-compare{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:15px}.tax-compare>div{border:1px solid var(--line);border-radius:11px;padding:14px}.tax-compare span,.tax-compare small{display:block;color:var(--mute);font-size:10.5px}.tax-compare b{display:block;font-family:var(--disp);font-size:20px;margin:4px 0}.limit-bar{height:18px;border-radius:100px;background:var(--wash);position:relative;margin:20px 0 5px;overflow:hidden}.limit-bar span{display:block;height:100%;background:var(--blue);border-radius:100px}.limit-bar i{position:absolute;top:-3px;width:2px;height:24px;background:var(--ink)}.limit-labels{display:flex;justify-content:space-between;font-size:10px;color:var(--mute)}.limit-labels b{color:var(--ink)}.warnbox{background:var(--no-w)!important;border-color:#efb2b2!important;color:var(--no)}.capital-dashboard{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-top:17px}.capital-dashboard>div{border:1px solid var(--line);border-radius:11px;padding:13px;background:#fff}.capital-dashboard span,.capital-dashboard small{display:block;font-size:10px;color:var(--mute)}.capital-dashboard b{display:block;font-family:var(--disp);font-size:22px;margin:3px 0}
@media(max-width:820px){.persistent-notice{top:58px}.recap-flow{display:grid;grid-template-columns:1fr}.recap-flow>i{display:none}.tax-compare,.capital-dashboard{grid-template-columns:1fr 1fr}.capital-dashboard>div:last-child{grid-column:1/-1}}
